...
def DvdIntoSubImg(I):
    ...
    return I.reshape(H, img_size[0], -1, img_size[1], c).swapaxes(1, 2).reshape(-1, img_size[0], img_size[1], c)
...
def MakeNet(nn = 'ocrnet'):
    ...
    return ANNC(img_size, ws, batchSize = 512, learnRate = 2e-5, maxIter = 64, name = nn, reg = 1e-5, tol = 1e-2, verbose = True, X = tf_img, Y = tf_targ)
